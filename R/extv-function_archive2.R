...
extv=function(tidy_set
              # ,geo_p=0.12,tem_p=0.35,bgt_p=1
              ,ran_p=0.2){
    ...
    `protocolData<-`(
      lapply(X=1,Y=.,function(X,Y){
        ...
        K=Z %>%
          ...
          # ungroup() %>%
          # mutate(
          #   tem=latest_date>=(tem-days(tem_d)) & latest_date<=(tem+days(tem_d))
          # ) %>%
          # select(-tem_d) %>%
          ...
          #     ungroup() %>%
          #     mutate(
          #       bt=latest_date>=(bt-days(bt_d)) & latest_date<=(bt+days(bt_d))
          #     ) %>%
          #     select(-bt_d) %>%
          #     filter(bt) %>%

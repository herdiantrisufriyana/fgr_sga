...
divnn_calibrator_evaluator=function(data
  ...
  prediction=
    ...
    predict_generator(
      ...
      ,steps=ceiling(ncol(data[['int']])/batch_size)
      ,verbose=1
    ) %>%
    ...
    lapply(function(x){
      x[seq(ncol(data[['int']])),,drop=F]
    }) %>%
    
    # Combine with the outcome data.
    c(list(outcome=data[['int']]$outcome)) %>%
    as.data.frame() %>%
    select(root,outcome) %>%
    mutate(
      outcome=

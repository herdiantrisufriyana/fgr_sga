...
dca=
  ...
  geom_text(
    data=
      ...
      left_join(
        data.frame(
          ...
          ,x_value=c(0.03,0.03,0.02)
          ,y_value=c(0.00125,0.00075,0.003)
        )
        ,by='model'
      )
    ...
    ,aes(
      x=x_value
      ,y=y_value
      ,label=paste0(round(th,2),', ',model)
      ,color=model
    )
    # ,size=3,hjust=-0.15,vjust=1,family='sans'
    ,size=2.5,hjust=0,vjust=0.5,family='sans'
    ,show.legend=F
  ) +
  # geom_vline(
  #   data=
  ...
  # ) +
  # geom_line() +
  geom_line(aes(color=model),na.rm=T,show.legend=F) +
  # geom_point() +
  geom_point(aes(color=model),na.rm=T,show.legend=F) +
  # facet_wrap(~model,ncol=5) +
  # scale_x_continuous(
  ...
  # ) +
  scale_x_continuous(
    'Threshold',limits=c(0,0.06),breaks=seq(0,0.1,0.01)
    ,labels=scales::number_format(accuracy=0.01,decimal.mark='.')
  ) +
  scale_y_continuous(
    'Net benefit'
...
auroc2= 
  ...
  filter(
    set
    %in%c('Internal calibration\nsplit'
          ,'External random &\nnon-random splits'
        )
  ) %>%
  ggplot(aes(model,Score,color=model_group)) +
  geom_point(size=1,show.legend=F) +
  geom_errorbar(aes(ymin=lb,ymax=ub),width=0.5,size=1,show.legend=F) +
  # geom_hline(aes(yintercept=set_score),lty=2) +
  # geom_hline(yintercept=0.5,lty=2) +
  # geom_text(
  #   aes(
  #     x=model
  #     ,y=ifelse(set=='Internal calibration\nsplit',0.55,0.65)
  #     ,label=model
  #   )
...
```{r Compare the AUROCs of our models only, fig.height=2.85827, fig.width=1.85}
auroc2
```

```{r Revise figure 4 to figure 3, include=FALSE}
srma_group3=
  srma_rename %>%
  select(
    -ultrasound,-biomarker,-others,-inaccessible,-element
    ,-event,-nonevent
    ,-sens,-sens_lb,-sens_ub,-spec,-spec_lb,-spec_ub,-auroc,-auroc_lb,-auroc_ub
    ,-inference
  ) %>%
  gather(predictor,used,-study,-year,-type, -total) %>%
  mutate_at(c('study','predictor'),function(x)factor(x,unique(x))) %>%
  mutate(
    predictor=
      predictor %>%
      str_replace_all('papp_a','PAPP-A') %>%
  ...
  left_join(
    read_xlsx('data/srma_study_model.xlsx') %>%
      rename(study2=study)
    ,by='study2'
  ) %>%
  mutate(
    study=
      citation %>%
      paste0(ifelse(study3=='','    ',paste0(', ', study3)))
  ) %>%
  select(-study2,-study3) %>%
  mutate(study=reorder(study,Score))
...
srma_group3=
  ...
  mutate(
    ...
    ,type2=
      case_when(
        predictor%in%c(
          filter(srma_group3,type=='Ultrasound only' & used==1)$predictor
          ,'EFW','MAP','NT'
        )
        ~'Ultrasound'
        ,predictor
         %in%c('PAPP-A','CRL','PlGF','sFLT-1','Free b-hCG','PP13','ADAM12')
         ~'Biomarker'
        ,predictor
         %in%filter(srma_group3,str_detect(type,'Ot|other') & used==1)$predictor
         ~'Others'
      ) %>%
      factor(c(
        'Ultrasound'
  ...
  mutate(used=ifelse(used==1,'yes','no')) %>%
  filter(!is.na(predictor))

auroc_plain=
  srma_group3 %>%
  select(study,type,Score,lb,ub) %>%
  unique() %>%
  ggplot(aes(study,Score)) +
  geom_point(size=1) +
  geom_errorbar(aes(ymin=lb,ymax=ub),width=0.5,size=1) +
  geom_vline(xintercept=seq(0.5,20.5,1),color='grey') +
  geom_hline(
    yintercept=
      auroc_table %>%
      filter(model=='DI-VNN' & set=='Internal calibration\nsplit') %>%
      pull(Score)
    ,lty=2
  ) +
  geom_hline(
    yintercept=
      auroc_table %>%
      filter(model=='DI-VNN' & set=='External random &\nnon-random splits') %>%
      pull(Score)
    ,lty=2
  ) +
  geom_hline(yintercept=0.5,lty=2) +
  geom_text(
    ...
    ,aes(
      label=
        ifelse(
          type=='Ultrasound only' & study=='Bano S et al. (2010), a'
          ,'DI-VNN (Internal calibration split)'
          ,NA
        )
      )
    ,size=2.5,hjust=0,vjust=-0.5,na.rm=T
  ) +
  geom_text(
    x=1
    ,y=
    ...
    ,aes(
      label=
        ifelse(
          type=='Ultrasound only' & study=='Bano S et al. (2010), a'
          ,'DI-VNN (External random & non-random splits)'
          ,NA
        )
      )
    ,size=2.5,hjust=0,vjust=-0.5,na.rm=T
  ...
  facet_grid(~type,scales='free',space='free',switch='both') +
  scale_y_continuous(
    'AUROC (95% CI)',breaks=seq(0,1,0.1)
    ,labels=scales::number_format(accuracy=0.1,decimal.mark='.')
  ) +
  theme(
    ...
    # ,axis.text.x=element_text(angle=90,vjust=0.5,hjust=1)
    ,axis.ticks.x=element_blank()
    ,axis.title.y=element_text(size=unit(10,'pt'),family='sans')
    ,axis.text.y=element_text(size=unit(8,'pt'),family='sans')
    ,strip.text.x=element_blank()
  )
...
feature_details=
  srma_group3 %>%
  ggplot(aes(study,predictor,fill=used)) +
  geom_tile(color='white') +
  facet_grid(type2~type,scales='free',space='free',switch='x') +
  theme(
    axis.title.x=element_blank()
    ,axis.text.x=element_blank()
    # ,axis.text.x=element_text(angle=90,vjust=0.5,hjust=1)
...
sampsize_details=
  srma_group3 %>%
  select(study,type,total) %>%
  unique() %>%
  ggplot(aes(study,total)) +
  geom_col(width=0.5) +
  geom_vline(xintercept=seq(0.5,20.5,1),color='grey') +
  geom_hline(yintercept=30529+143,lty=2) +
  geom_text(
    x=1
    ,y=log2(30529+143)
    ,aes(
    ...
    ,size=2.5,hjust=0,vjust=1.25,na.rm=T
  ) +
  facet_grid(~type,scales='free',space='free',switch='both') +
  scale_y_continuous(
    'Sample size',trans='log2'
    ,labels=scales::number_format(accuracy=1,decimal.mark='.',big.mark=',')
  ) +
  theme(
    panel.grid.major.x=element_blank()
    ,axis.title.x=element_blank()
    # ,axis.text.x=element_blank()
    ,axis.text.x=
      element_text(size=unit(8,'pt'),family='sans',angle=90,vjust=0.5,hjust=1)
    ,axis.ticks.x=element_blank()
    ,axis.title.y=element_text(size=unit(10,'pt'),family='sans')
    ,axis.text.y=element_text(size=unit(8,'pt'),family='sans')
    ,strip.text.x=element_blank()
...
```{r Plot the AUROCs with features, include=FALSE}
auroc_features=
  ggarrange(
    ggarrange(
      NULL
    ...
    ,ggarrange(
      ...
      ,NULL
      ,ncol=3,nrow=1
      ,widths=c(0.9-0.185,6.08661+0.185,0.3)
    )
    ,ncol=1,nrow=3
    ,heights=c(1.7,4.08661,2.8)
    # ,labels=letters[1:3]
  )
```

```{r Compare the AUROCs with features, fig.height=8.58661, fig.width=7.08661}
auroc_features
```
...
```{r figure-3, echo=FALSE, fig.height=6.85827, fig.width=7.08661}
# fig.height=5.85827, fig.width=7.08661 on top of this chunk if shown here
suppressWarnings(ggarrange(
    ggarrange(
      calibration_plot
    ...
    ,ggarrange(
      ...
      ,roc2 %>%
        ...
        mutate(set='Internal calibration\nsplit') %>%
        ggplot(aes(spec,sens,color=year)) +
        geom_vline(xintercept=c(0.95),lty=2) +
        geom_abline(slope=1,intercept=1,lty=2) +
        geom_line(aes(color=study),show.legend=F) +
        geom_text(
          data=
            spec_th %>%
            filter(!model%in%c('Causal RR','PC-RF')) %>%
            ...
            left_join(
              data.frame(
                model=c('PC-ENR','PC-GBM','DI-VNN')
                ,x_value=c(0.925,0.925,0.925)
                ,y_value=c(0.9-0.07,0.9-0.14,0.9)
              )
              ,by='model'
            ) %>%
            rename(study=model,sens=tpr,spec=tnr)
        ...
        scale_y_continuous('Sensitivity',limits=c(0,1))
      ,auroc2
      ,widths=c(3-0.6,2.58661+0.25,1.5+0.35)
      ,labels=c('b','c','')
      ,ncol=3,nrow=1
    )
    ,heights=c(4,2.85827)
  ...
  ggsave(
    filename="doc/figure2.eps"
    ,device=cairo_ps
    ,width=170
    ,height=170/7.08661*6.85827#170/7.08661*5.85827
    ,unit='mm'
  )
```
...
auroc_features +
  ggsave(
    filename="doc/figure3.eps"
    ,device=cairo_ps
    ,width=170
    ,height=170/7.08661*8.58661
    ,unit='mm'
  )
```
...
```{r Prepare ROC tables, eval=FALSE, include=FALSE}
...
```{r Load pre-obtained non-calib pred proba of DI-VNN, include=FALSE}
predictions=readRDS('data/predictions.rds')

# Prepare ontoarray prerequisites
test_data_int=
  readRDS('data/pw_hlin.rds') %>%
  .[,!pData(phenoData(.))$censoring] %>%
  .[,pData(protocolData(.))$int] %>%
  test_transformer(
...
```{r Reformat non-calib pred proba of DI-VNN, include=FALSE}
predictions_non_calib0=
  ...
  # Set back number of instances, that is changed due to batch sizing
  lapply(function(x)x[seq(ncol(test_data_int)),,drop=F]) %>%
  
  # Combine with the outcome data.
  c(list(outcome=test_data_int$outcome)) %>%
  as.data.frame() %>%
  select(root,outcome) %>%
  mutate(
    outcome=
  ...
  # Give back the rownames of the validation set
  `rownames<-`(colnames(test_data_int))

predictions_non_calib=
  predictions_non_calib0 %>%
  select(obs=outcome,pred=event)
```

```{r Obtain calib pred proba of DI-VNN, include=FALSE}
predictions_calib0=
  predictions_non_calib0 %>%
  cbind(
    suppressWarnings(
  ...
  select(-nonevent,-event) %>%
  rename_all(function(x)str_remove_all(x,'2')) %>%
  select(nonevent,event,everything())
...
```{r Show training samples with best predictions, eval=FALSE, include=FALSE}
pw_hlin %>%
  ...
  inner_join(
    predictions_calib0 %>%
      rownames_to_column(var='id') %>%
      select(id,outcome,pred=event)
    ,by=c('id','outcome')
  ) %>%
  inner_join(
    predictions_calib %>%
      rownames_to_column(var='id') %>%
      select(id,outcome=obs,calib_pred=pred)
    ,by=c('id','outcome')
  ) %>%
  select(
    subject_id
  ...
  mutate(
    outcome=as.integer(outcome=='event')
    ,pred=as.integer(pred>filter(spec_th,model=='DI-VNN')$th)
    ,calib_pred=as.integer(pred>filter(spec_th,model=='DI-VNN')$th)
  ) %>%
  filter(outcome==1 & pred==1 & calib_pred==1) %>%
  
  # Save these training examples with the best predictions.
  saveRDS('data/subject_examples.rds')
```

```{r Load training samples with best predictions & visit data, include=FALSE}
subject_examples=readRDS('data/subject_examples.rds')
target_visits=readRDS('data/target_visits.rds')
```

```{r Identify number of medical histories, include=FALSE}
med_hist_n=
  seq(nrow(subject_examples)) %>%
  pbsapply(function(x)
    target_visits %>%
      filter(
        subject_id==subject_examples$subject_id[x]
        & healthcare_id==subject_examples$healthcare_id[x]
      ) %>%
...
```{r Processing codes at the server side, eval=FALSE, include=FALSE}
